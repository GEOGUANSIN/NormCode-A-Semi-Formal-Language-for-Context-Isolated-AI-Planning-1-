...
All tool calls emit these generic events via `MonitoredToolProxy`:
...
`tool_name` is one of: `llm`, `file_system`, `python_interpreter`, `prompt`, `user_input`, `chat`, `canvas`, `parser`, `shell`, `search`, `git`, `editor`, `memory`.

#### Code Assistant Tool Events

In addition to generic tool events, code assistant tools emit domain-specific events:

| Event | Payload | Description |
|-------|---------|-------------|
| `shell:started` | `{command, cwd}` | Shell command execution started |
| `shell:completed` | `{command, exit_code, stdout, stderr}` | Shell command completed |
| `shell:failed` | `{command, error}` | Shell command failed |
| `shell:background_started` | `{pid, command}` | Background process started |
| `shell:background_stopped` | `{pid}` | Background process stopped |
| `search:grep_started` | `{pattern, path}` | Grep search started |
| `search:grep_completed` | `{pattern, match_count}` | Grep search completed |
| `search:glob_completed` | `{pattern, file_count}` | Glob search completed |
| `git:status` | `{branch, staged, modified, untracked}` | Git status retrieved |
| `git:diff` | `{path, staged, diff_length}` | Git diff retrieved |
| `git:committed` | `{hash, message}` | Git commit created |
| `git:branch_changed` | `{branch, created}` | Branch switched or created |
| `editor:edit_started` | `{path, operation}` | File edit started |
| `editor:edit_completed` | `{path, operation, lines_changed}` | File edit completed |
| `editor:file_created` | `{path}` | New file created |
| `editor:edit_failed` | `{path, error}` | File edit failed |
| `memory:search:start` | `{query, category}` | Memory search started |
| `memory:add:start` | `{category, tags}` | Memory store started |
...
The `AgentConfig` includes a `tools` field with `AgentToolsConfig` for code assistant tools. Each tool config has an `enabled` flag and tool-specific settings. The `ToolCallEvent.tool_name` field now includes `"shell"`, `"search"`, `"git"`, `"editor"`, and `"memory"` in addition to the original core tools.

Tool calls from code assistant tools appear in the Agent Panel's tool call feed with distinct color chips:

| `tool_name` | Chip Color |
|-------------|------------|
| `llm` | Blue |
| `file_system` | Green |
| `python` | Yellow |
...
interface AgentToolsConfig {
  ...
  git?: GitToolConfig;
  editor?: EditorToolConfig;
}

interface ShellToolConfig {
  enabled: boolean;
  timeout: number;         // Default command timeout in seconds
}
...
interface GitToolConfig {
  enabled: boolean;
  allow_commits: boolean;  // Whether the agent can create commits
}

interface EditorToolConfig {
  enabled: boolean;
  base_dir?: string;       // Working directory for file operations
}
...
interface ToolCallEvent {
  ...
  flow_index: string;
  agent_id: string;
  tool_name: string;       // "llm" | "file_system" | "shell" | "search" | "git" | "editor" | ...
  method: string;
