...
NormCode occupies a unique position in the AI tooling landscape:

| Approach | What You Get | What You Give Up |
|----------|-------------|-----------------|
| **Raw AI Models** (Claude, GPT) | Flexible, powerful | No structure, no auditability — you adapt to how the model works |
| **Agent Code Frameworks** (LangChain, CrewAI) | Programmable workflows | Only developers can read/change them. Implicit data flow. |
| **Workflow UIs** (Dify, Coze) | Visual, low-code | Locked UIs, not suited for complex multi-step scenarios |
| **NormCode** | Structured, auditable, readable | Requires learning the syntax (minimal — two symbols) |

NormCode is a **semi-formal language**: structured enough for reliable machine execution, readable enough for human review and approval. Plans can be written by hand, generated by AI, or a mix of both.

This positions NormCode for **high-compliance domains** — financial risk assessment, legal review, enterprise decision systems — where you need to prove exactly what the AI did at each step.

For the complete ecosystem picture (6 products, 3 roles, deployment lifecycle), see the [Ecosystem Overview](ecosystem.md).
...
NormCode serves [three roles](ecosystem.md#three-roles-six-products):

| Role | Profile | Entry Point |
|------|---------|-------------|
| **Agent Designer** | Developers/engineers designing AI workflows | [Grammar](../2_grammar/README.md), [Compilation](../4_compilation/README.md) |
| **Product Operator** | Managers/ops teams inspecting, debugging, deploying | [Canvas](../5_tools/README.md), [Server](../5_tools/normcode_server_guide.md) |
| **End User** | Domain experts using finished AI agents | Purpose-built client interfaces |
...
This overview covers NormCode's core philosophy. The ecosystem extends to six products across the full AI agent lifecycle:

**Language → Compiler → Agent → Canvas → Server → Clients**

From describing a plan in natural language, to compiling it, to executing and debugging it visually, to deploying it for end users — each stage has a dedicated product.

See **[Ecosystem Overview](ecosystem.md)** for the complete product landscape, lifecycle stages, and technical properties.

---

## Next Steps

- **[Ecosystem](ecosystem.md)** - The full product landscape and lifecycle
...
- **[Built-in Plans](../6_normcode_built-in/README.md)** - Real-world plans: Canvas Assistant, PPT generation, meta-compiler
